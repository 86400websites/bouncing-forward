...
- [ ] Security hardening deferrals (rate limiting, CAPTCHA, etc.) are marked **required-before-scale**, not optional. But **basic** public-form abuse control (a server-verified bot check and/or rate limit) is **launch-blocking** and may not be parked here — only _advanced_ hardening (tighter limits, anomaly detection) is scale-deferrable (`docs/SECURITY-CHECKLIST.md` §5).
