...
The rules for handling configuration values and secrets on Bouncing Forward. Read this once before touching any key; follow the procedures every time after.
...
| Database URL _(Supabase example)_    | `NEXT_PUBLIC_SUPABASE_URL`             | Public            | Local + approved host environments                 |
| Publishable key _(Supabase example)_ | `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | Public            | Local + approved host environments                 |
| Email provider key _(example)_       | `[EMAIL_PROVIDER_API_KEY_NAME]`        | Private           | Trusted local/server secret stores                 |
| Database secret key _(optional)_     | `[DATABASE_SECRET_KEY_NAME]`           | Private           | Trusted server secret store; avoid unless required |
| Bot-protection secret _(example)_    | `[BOT_PROTECTION_SECRET_NAME]`         | Private           | Trusted local/server secret stores                 |
