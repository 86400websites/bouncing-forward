...
**Choosing:** if the run should be _repeatable evidence_ or needs console/network eyes → Playwright MCP. If you just need to _go look at something fast_ → Agent Browser. When in doubt for Part 2 evidence, use Playwright MCP.
...
- Never bypass bot protection or rate limiting — verifying rejection _is_ the test.
