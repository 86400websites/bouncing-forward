...
Every change to `Bouncing Forward` follows:
...
The independent verdict is a **gate, not a suggestion.** The reviewer is "advisory" only in that it never _acts_ — it does not merge, push, or edit. It does **not** mean the owner may merge over its findings: a **Blocking** finding is never merged; a **Should-fix** may be deferred only with a logged owner + reason (PROJECT-STATUS §8). "Done" is never reached with an unresolved Blocking finding.
