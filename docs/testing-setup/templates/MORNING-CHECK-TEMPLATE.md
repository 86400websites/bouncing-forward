...
Do not panic-fix on `main`. Say: _"/handle-error — the morning check failed today, here's the email."_ It follows the normal incident lane (`docs/error-tracking/ERROR-TRACKING-GUIDE.md`): understand → severity → fix sprint → the fixed thing gets a stronger test.
