...
| Preview hosts and protection                       | PR #36 Preview verified 9 Sep 2026: unique deployment `bouncing-forward-2d25v8xkf-86400-s-projects.vercel.app` (branch alias `bouncing-forward-git-codex-testing-readiness-86400-s-projects.vercel.app`, team slug `86400-s-projects`), commit `654444d`, environment `preview`, production address `www.bouncing-forward.com`. Protection = Vercel Authentication (unauthenticated clients are redirected to `vercel.com/sso-api`); the harness admits itself with `VERCEL_AUTOMATION_BYPASS_SECRET` sent once via Vercel's cookie hand-off, only to the verified origin, never across redirects. The Vercel API cross-check (`VERCEL_TOKEN` + `VERCEL_PROJECT_ID`) stays optional |
...
### Proof record — 8–9 September 2026 (Phase 0; Preview evidence on candidate `654444d`)
...
| ------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Preflight     | PASS on Preview (9 Sep 2026, `654444d`, dpl_BBXeXNMQcNrEk9Ar1tm8r1z2rJQX)      | `/api/health` reported environment preview, commit `654444d`, branch `codex/testing-readiness`, production address `www.bouncing-forward.com`, public Supabase ref `hmcojplrqoqhyigvtgyp`, privileged key present (opaque format — project proven by P5), Stripe key mode test, price + webhook secret + Mailchimp + access codes + contact endpoint configured; protection on and bypassed. Offline refusals (no-target, Production hosts, foreign hosts, missing SHA, bypass on Production, PROD Supabase ref, live Stripe key, URL with query) exercised 8–9 Sep. |
| P1            | PASS (public half + fixture sign-in, Preview `654444d`) · privileged half → P5 | `@proof P1a`: the served bundle names only the TEST project. Both TEST fixture accounts signed in on the Preview (free → "No Book Package yet"; premium → "The Book Package"). Production public half observed read-only 9 Sep (`ilnoazendkckzhbuzias`). The privileged key's project is proven when P5 writes the entitlement.                                                                                                                                                                                                                                      |
...
| P6            | PASS (reachability, Preview `654444d`) · business effect → P5                  | `@proof P6a`: one unsigned request through the bypass answered 400 "Invalid signature" (secret configured). Production: `www` route answers, apex answers 308 (observed read-only 9 Sep). Sandbox endpoint destination for the Preview still to be created by the owner.                                                                                                                                                                                                                                                                                             |
...
| P10           | Preview half PASS · Production half observed                                   | Preview: Vercel Authentication on; unauthenticated client redirected to Vercel sign-in; sanctioned bypass accepted (9 Sep 2026). Production: read-only GET 9 Sep — 200, Vercel-served, security headers present (§3).                                                                                                                                                                                                                                                                                                                                                |
| P11           | PASS (Preview `654444d`)                                                       | `/` and `/api/health` answered on the Preview through the sanctioned bypass; local `next start` also answered on 9 Sep. Rechecked by the preflight on every run.                                                                                                                                                                                                                                                                                                                                                                                                     |
| P12           | PARTIAL (automated half done) · owner half PENDING                             | The preflight confirmed the Preview's TEST Supabase ref and test-mode Stripe key by class. Owner still confirms in the dashboards: Stripe test price/webhook belong to this site, Preview `PREMIUM_ACCESS_CODES`, Mailchimp/Formspree shared by decision (§9).                                                                                                                                                                                                                                                                                                       |
| P13           | Launch                                                                         | Unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

### Proof record — 10 September 2026 (Phase 2 candidate `aaf502d`, gate alias)

Candidate: branch `claude/t1-gate-suite`, base `07ef8c3` (= `main`), head `aaf502d18b3fb73e421ed5362bb77510b18911b6`, deployed at the branch alias `bouncing-forward-git-claude-t1-gate-suite-86400-s-projects.vercel.app` (the long-lived gate address). Preflight PASS: environment preview, commit `aaf502d`, project identity, public + privileged Supabase = TEST (privileged key opaque → proven by P5 in Phase 3), Stripe test mode with price and webhook secret configured, Mailchimp / access codes / contact endpoint configured, protection on and bypassed with the rotated secret. P1a PASS, P6a PASS, SM-001 PASS (desktop + 390 px). P2–P5, P7 (§9 manual), P8b, P9, P12 owner half: pending the Phase 3 full run and the owner's dashboard checks (MN-007). Sandbox webhook destination for this alias confirmed read-only through the harness key (one enabled test-mode endpoint, both checkout events).

### Proof record — 10–11 September 2026 (Phase 3 full gate run, candidate `8b5460c`, gate alias)

Candidate: branch `claude/t1-gate-suite`, head `8b5460c0f1cb31774d6576d263ded4fee3b955c5`, served by the
gate alias. Three full runs (19:04–20:04 UTC on 10 September was the final one); report
`docs/test-reports/2026-09-11-test-report.md`. Preflight PASS on every run.

| ID            | State                                             | Evidence / next action                                                                                                                                                                                                                                               |
| ------------- | ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1            | **PASS (both halves)**                            | Public half by `@proof P1a`; the privileged half is now proven by P5 — the webhook wrote the TEST entitlement for a real test purchase (PY-001), which only the TEST secret key can do.                                                                              |
| P2            | **PASS (TEST half)** · PROD negative → owner      | AC-001 created a throwaway account in TEST and the `cleanup` project removed it; every run record shows 0 not-cleaned, 0 errors. The Production negative control stays an owner read-only check (MN-007).                                                            |
| P3            | **PASS (return path)** · delivery → MANUAL        | AC-003c proved the recovery link resolves to this Preview and opens "Choose a new password."; AC-003d completed the password change and re-login. Email delivery itself is MN-002.                                                                                   |
| P4            | **PASS**                                          | PY-001 drove hosted Checkout in test mode at $9.99; PY-011 confirmed the test banner, one-time price and sandbox endpoint. No live event.                                                                                                                            |
| P5            | **PASS**                                          | After PY-001 the TEST entitlement row existed and both paid PDFs downloaded for that account; PY-003 (targeted re-run) proved nothing opens before the webhook lands. No corresponding PROD write (owner check, MN-007).                                             |
| P6            | **PASS (reachability + business effect)**         | `@proof P6a` plus PY-008 (unsigned/forged refused), PY-012 (foreign-site events answered "ignored", nothing written or labelled), PY-014 (a failed save answers 500 so Stripe retries).                                                                              |
| P7            | NOT ISOLATED (owner decision, unchanged)          | Mailchimp, Formspree and the access codes stay shared. One labelled contact message and one controlled identity per run, all registered and cleaned. **New finding:** the cleanup archives the stable identity and the site cannot re-subscribe it — see the report. |
| P8b           | **PASS**                                          | IN-008: an account reads only its own entitlement row; other and anonymous reads return nothing; inserts and updates through the public key are refused. TEST only.                                                                                                  |
| P10           | Preview half **PASS** · Production half at launch | Protection on; the sanctioned bypass admitted every context; no bypass value appears in any recorded evidence.                                                                                                                                                       |
| P11           | **PASS**                                          | Rechecked by the preflight on all three runs.                                                                                                                                                                                                                        |
| P12           | Automated half **PASS** · owner half PENDING      | The preflight confirmed the TEST Supabase ref and test-mode Stripe key by class on each run. Owner dashboard confirmation is still MN-007.                                                                                                                           |
| P8 · P9 · P13 | Unchanged                                         | P8 PARTIAL and P9 PENDING as recorded above; P13 belongs to launch.                                                                                                                                                                                                  |
