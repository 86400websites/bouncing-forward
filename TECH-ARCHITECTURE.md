...
**Purpose:** the default technical architecture for every website built on this system. This document is the source of truth for *what we build on*. When any other doc disagrees with this one about the stack, this one wins. Hand it to any AI tool, designer, or developer to scaffold a new project consistently.

> Process (branch → PR → Preview → merge) lives in [`WORKFLOW.md`](./WORKFLOW.md). Visual language (type, color, motion) lives in [`DESIGN.md`](./DESIGN.md). This file is the technical layer between them.
...
- **Identifiers:** `bouncing-forward`, `Bouncing Forward`, `Maher Kaddoura`, `[PRODUCTION_DOMAIN]`, `bouncing-forward`, `[VERCEL_TEAM_SLUG]`, `[SUPABASE_PROJECT_REF]`.
...
- **Audience & offer:** `people navigating profound loss or hardship who want a practical way forward` and `the Bouncing Forward book, the free Taking Stock Inventory, and the Compass & Path Check` shape routes, copy, structured-data types, and the SEO surface.
