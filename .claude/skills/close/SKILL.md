...
This is the bookend to `/sprint-prompt`: that skill _opens and records_ a sprint; this one _verifies and hands off_. It does not replace the per-step gating during the sprint — it is the single final sweep.
...
- From the diff, decide which sections below apply — and **when in doubt, run the section**. Triggers:
  - **Database** — whenever the diff changes _or implies_ a database contract: any change under the project's migrations directory (per `docs/TECH-ARCHITECTURE.md`; e.g. `supabase/sql/**`), **or** app code that reads/writes a table, column, or RPC. A required migration can be missing from the diff — that omission is itself the finding. Skip if the project has no database.
